# OptSurvCutR: OptSurvCutR: Validated Cut-Point Selection for Survival Analysis
...
cut-points ('find_cutpoint_number()'), locate them using systematic or
genetic algorithms (via 'rgenoud'), and validate stability via
bootstrapping ('validate_cutpoint()'). Analyses can be adjusted for
covariates using 'survival' package models. Ideal for biomarker analysis
and patient stratification where non-linear relationships may exist.
