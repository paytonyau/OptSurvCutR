...
# Ensure optional dependency is available for genetic tests
...
# Small dataset with constant predictor
...
# --- 4. Pre-Calculated Results for Heavy Tests ---
# This prevents validation and plotting tests from having to re-run
# the find_cutpoint algorithm hundreds of times, drastically speeding up tests.
...
# A clean, globally available result object
...
# Failsafe if the global setup breaks
if (any(is.na(valid_fc_result_for_boot$optimal_cuts))) {
  warning("Helper setup failed: Could not generate a valid valid_fc_result_for_boot for tests.")
}
