...
test_that(".calc_ic executes all fallback branches and mathematical edge cases", {
  ...
  for (crit in c("AIC", "AICc", "BIC")) {
    expect_true(is.numeric(.calc_ic(model = list(loglik = c(0, -50)), k = 2, n = 20, criterion = crit)))
  }

  # Trigger NULL and missing data structures
  expect_true(is.na(.calc_ic(model = NULL, k = 2, n = 20, criterion = "BIC")))
  expect_true(is.na(.calc_ic(model = list(), k = 2, n = 20, criterion = "BIC")))
  expect_true(is.na(.calc_ic(model = list(loglik = c(NA, NA)), k = 2, n = 20, criterion = "BIC")))

  # Trigger BIC sample size failures
  expect_true(is.na(.calc_ic(model = list(loglik = c(0, -50)), k = 2, n = NA, criterion = "BIC")))
  expect_true(is.na(.calc_ic(model = list(loglik = c(0, -50)), k = 2, n = 0, criterion = "BIC")))

  # Trigger AICc saturation path (n - k - 1 <= 0)
  expect_true(is.na(.calc_ic(model = list(loglik = c(0, -50)), k = 4, n = 5, criterion = "AICc")))
})
...
test_that(".validate_find_cutpoint_inputs traps argument discrepancies early", {
  ...
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 1, method = "INVALID", "logrank", NULL))
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 1, method = "systematic", "INVALID", NULL))

  # Explicit case sensitivity testing validation for {G2.3a} / {G2.3b}
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 1, method = "Systematic", "logrank", NULL))
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 1, method = "systematic", "Logrank", NULL))

  # Trigger num_cuts integer and constraint validation checks
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = -1, "systematic", "logrank", NULL))
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 1.5, "systematic", "logrank", NULL))
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 2, "systematic", "hazard_ratio", NULL))
  expect_error(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 3, "systematic", "logrank", NULL))

  # Trigger missing assignment boundaries
  expect_error(.validate_find_cutpoint_inputs(df, NULL, "time", "status", num_cuts = 1, "systematic", "logrank", NULL))
  expect_error(.validate_find_cutpoint_inputs(df, "pred", NULL, "status", num_cuts = 1, "systematic", "logrank", NULL))

  # Trigger column existence mismatch errors
  expect_error(.validate_find_cutpoint_inputs(df, "MISSING", "time", "status", num_cuts = 1, "systematic", "logrank", NULL))

  # Clean pass check via robust expect_silent matrix evaluation
  expect_silent(.validate_find_cutpoint_inputs(df, "pred", "time", "status", num_cuts = 1, "systematic", "logrank", "cov"))
})
...
test_that(".prepare_cutpoint_data subsets, renames, and omits NA cases", {
  ...
  cleaned <- .prepare_cutpoint_data(df_na, "pred", "time", "status", "cov")
...
test_that(".validate_data_conditions monitors sample boundaries and quiet modes completely", {
  ...
  expect_error(
    .validate_data_conditions(bad_time, nmin = 1, num_cuts = 1, "event", quiet = TRUE),
    regexp = "Time"
  )
  ...
  res_prop <- suppressMessages(
    .validate_data_conditions(clean_df, nmin = 0.25, num_cuts = 1, "event", quiet = TRUE)
  )
  ...
  expect_error(.validate_data_conditions(clean_df, nmin = -0.5, num_cuts = 1, "event", quiet = TRUE))

  # Scenario D: Insufficient global sample space allocation checks
  res_low_data <- .validate_data_conditions(clean_df, nmin = 3, num_cuts = 1, "event", quiet = TRUE)
  ...
  res_few_unique <- .validate_data_conditions(constant_df, nmin = 1, num_cuts = 1, "event", quiet = TRUE)
