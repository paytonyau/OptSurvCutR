...
```{=html}
<style>
/* Restrict blockquote styling to document text bodies to shield gridtext */
...
> **💡 Key Insight: Multi-State Endpoints & Informative Censoring**
> In the raw `survival::pbc` dataset, patient status is recorded with three discrete outcomes: `0` (alive/censored), `1` (received liver transplant), and `2` (deceased). `OptSurvCutR` models standard single-event survival data and requires a binary event indicator (`0` vs `1`).
>
> In end-stage liver disease, transplantation is allocated preferentially to the highest-risk patients. Treating transplant recipients as right-censored would introduce substantial **informative censoring bias**, artificially deflating observed hazard rates in higher bilirubin tiers. We therefore define a composite **Transplant-Free Survival (TFS)** endpoint where both death and transplantation are treated as events (`1`), while patients surviving event-free with their native liver remain censored (`0`).

> **💡 Key Insight: Managing Skewed Distributions & Confounders**
> Raw clinical laboratory values are often heavily right-skewed. Because `OptSurvCutR` evaluates survival using rank-based statistics (such as the Log-rank and Cox score tests), it relies on the *rank order* of patients rather than absolute numerical distances. This makes the algorithm mathematically robust against distributional skewness.
>
...
Following bootstrap calculations, `OptSurvCutR` evaluates both **Precision** (Confidence Interval width relative to the overall data range) and **Validity** (absence of interval overlap), grading model health into four distinct performance tiers:
