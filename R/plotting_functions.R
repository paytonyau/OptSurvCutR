...
plot.find_cutpoint <- function(
  ...
  if (is.null(x$optimal_cuts) || anyNA(x$optimal_cuts)) {
...
#' @param x A validation result object for generic printing or plotting methods dispatch.
#' @param object A validation result object passed to down-stream summary tracking.
#' @param show_descriptives Logical. Show complete bootstrap distribution descriptives? Defaults to \code{TRUE}.
#' @param show_ci Logical. Print 95\% Empirical Confidence Interval boundaries? Defaults to \code{TRUE}.
#' @param show_params Logical. Display original metadata loop execution parameters? Defaults to \code{TRUE}.
#' @param plot.it Logical. If \code{TRUE}, automatically outputs the visual sampling line charts. Defaults to \code{FALSE}.
#' @param ... Additional arguments passed down to downstream rendering pipelines.
