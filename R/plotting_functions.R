# ===================================================================
# PLOTTING FUNCTIONS
# Diagnostic, publication-ready, and interactive plots.
# ===================================================================
...
#' aesthetic for all OptSurvCutR plots. Now features subtle light-grey
...
#'
#' @description
#' Wraps static `ggplot` or `ggsurvplot` objects into interactive HTML
#' widgets using `plotly`.
#'
#' @param p A plot object generated by OptSurvCutR.
#' @return A `plotly` interactive widget.
#'
#' @importFrom cli cli_abort
#' @export
optsurv_interactive <- function(p) {
  if (!requireNamespace("plotly", quietly = TRUE)) {
    cli::cli_abort(c("Package {.pkg plotly} is required for interactivity.",
                     "i" = "Install it using: install.packages('plotly')"))
  }

  if (inherits(p, "ggsurvplot")) {
    return(plotly::ggplotly(p$plot))
  }
...
#' @param type Plot framework type: `"outcome"`, `"distribution"`, `"forest"`,
#'    `"surface"`, `"trajectory"`, `"diagnostic"`, or `"landmark"`.
#' @param return_data Logical. If `TRUE`, exits the router early and returns the
#'    assigned underlying data frame template.
#' @param landmark Numeric. The operational milestone timestamp used if `type = "landmark"`.
...
plot_optimisation_curve <- function(cutpoint_result, ...) {
  ...
  if (params$num_cuts == 1) {
    ...
    if (!is.na(optimal_cut)) {
      p + ggplot2::geom_vline(xintercept = optimal_cut, linetype = "dashed",
...
#' @return A publication-ready `ggplot` canvas frame.
#'
...
#'   defaults to 20% of maximum follow-up.
#' @param ... Additional arguments passed down to internal rendering pipelines.
#'
#' @return A ggsurvplot or ggplot object representing landmark stratification curves.
#' @export
...
plot_validation <- function(validation_result,
  ...
  if (num_available_cuts == 1) {
    ...
    p <- ggplot2::ggplot(log_df, ggplot2::aes(x = .data[[col_x_name]], y = .data[[col_y_name]])) +
      ...
      # Layer 3: Target Baseline Crosshair Anchor
...
.plot_km_curve <- function(x, df, title = "Kaplan-Meier Survival Estimation",
  ...
  dynamic_palette <- if (num_strata <= 4) {
    ...
    # Fallback to a high-contrast editorial spectrum if cuts exceed 3
...
.plot_hr_forest <- function(x, df, reference_group = "G1", main = "Adjusted Clinical Risk Profile", ...) {
  ...
  for (f_row in seq_len(nrow(forest_df))) {
    ...
    if (!is.na(current_gid)) {
      # This is a biomarker row; pull true matching cross-counts
      ...
      # This is a covariate model row; populate with zeros to keep space alignment clean
      bar_list[[b_idx]] <- data.frame(
        Variable = rep(current_var, 2),
        Value    = c(0, 0),
        Metric   = c("Survivors/Censored", "Observed Events (Deaths)")
